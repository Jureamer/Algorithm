...
function solution(arr){
  ...
  for(let i = 0; i < arr.length; i++) {
    ...
    if(!answer[0]) { // answer에 아이가 없을 때
      if(arr[i] > arr[i+1]) { // 뒷 번호 아이보다 키가 클 경우
        answer.push(i+1); // 정답에 숫자를 체크
      }
    }
    
    if(answer[0] < i) { // answer에 현수가 들어왔을 때
      if(arr[i] < arr[i-1]) { // 앞의 번호 아이보다 키가 작은 아이를 찾는다.
        answer.push(i+1);
      }
    }
  }
